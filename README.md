...
> Based on [github.com/future-architect/go-mcprotocol.git](https://github.com/future-architect/go-mcprotocol.git)
...
The client uses a **persistent TCP connection** (长连接). Once connected, all subsequent `Read`/`Write`/`HealthCheck` operations reuse the same connection.
...
```go
	...
	defer client.Close() // Close the connection when done
	
	// All operations reuse the same connection
	client.HealthCheck()
	client.Read("D", 100, 3)
	client.Write("D", 100, 3, writeData)
```
